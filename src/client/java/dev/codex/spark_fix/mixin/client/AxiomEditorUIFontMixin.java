...
import dev.codex.spark_fix.AxiomFontToast;
...
abstract class AxiomEditorUIFontMixin {
    private static final Logger SPARK_FIX_LOGGER =
        LoggerFactory.getLogger("spark_fix/AxiomFonts");
    ...
    private static boolean sparkFix$recoverFailedChineseAtlas(ImFontAtlas atlas) {
        ...
        try {
            ...
            if (result == null) {
                SPARK_FIX_LOGGER.error("Axiom font recovery could not build even the minimal atlas.");
            ...
            SPARK_FIX_LOGGER.error("Unexpected error while rebuilding Axiom's font atlas.", exception);
            AxiomFontToast.showFailure();
            return false;
        }
    }
